...
const internalCommunicatorFacade = new InternalCommunicatorFacade(
    Config.COMMUNICATOR_TYPE
);
...
webSocketServer.on(WebSocketServer.CLIENT_MESSAGE_EVENT, (clientId, data) => {
    try {
        ...
        Utils.forEach(messages, (message) => {
            ...
            if (
                normalizedMessage.type === MessageUtils.HEALTH_CHECK_TYPE ||
...
webSocketServer.on(
    WebSocketServer.CLIENT_DISCONNECT_EVENT,
    async (clientId) => {
...
internalCommunicatorFacade.on(InternalCommunicatorFacade.AVAILABLE_EVENT, () =>
    messageBuffer.enablePolling()
);
internalCommunicatorFacade.on(
    InternalCommunicatorFacade.NOT_AVAILABLE_EVENT,
    () => messageBuffer.disablePolling()
);
...
function processTopicTypeMessage(clientId, message) {
    switch (message.action) {
        ...
        default:
            respondWithFailure(
                clientId,
                `Unsupported topic message action: ${message.action}`,
                message
            );
            break;
    }
...
function processPluginTypeMessage(clientId, message) {
    switch (message.action) {
        ...
        default:
            respondWithFailure(
                clientId,
                `Unsupported plugin message action: ${message.action}`,
                message
            );
            break;
    }
...
function processNotificationTypeMessage(clientId, message) {
    switch (message.action) {
        ...
        default:
            respondWithFailure(
                clientId,
                `Unsupported notification message action: ${message.action}`,
                message
            );
            break;
    }
...
function processTopicCreateAction(clientId, message) {
    if (message.payload && Array.isArray(message.payload.topicList)) {
        internalCommunicatorFacade
            .createTopics(message.payload.topicList)
            .then((createdTopicList) => {
                ...
                logger.info(
                    `Topics ${createdTopicList} has been created (request from ${clientId})`
                );
            })
            .catch((error) =>
        ...
        respondWithFailure(
            clientId,
            `Payload should consist an array with topics to create`,
            message
        );
    }
}
...
function processTopicSubscribeAction(clientId, message) {
    if (message.payload && Array.isArray(message.payload.topicList)) {
        internalCommunicatorFacade
            .subscribe(
                clientId,
                message.payload.subscriptionGroup,
            ...
            .then((topicSubscriptionList) => {
                ...
                logger.info(
                    `Client ${clientId} has been subscribed to the next topics: ${topicSubscriptionList}`
                );
            })
            .catch((error) =>
                respondWithFailure(clientId, error.message, message)
        ...
        respondWithFailure(
            clientId,
            `Payload should consist property "t" with list of topics to subscribe`,
            message
        );
    }
}
...
function processTopicUnsubscribeAction(clientId, message) {
    if (message.payload && Array.isArray(message.payload.topicList)) {
        internalCommunicatorFacade
            .unsubscribe(clientId, message.payload.topicList)
            .then((topicUnsubscriptionList) => {
                ...
                logger.info(
                    `Client ${clientId} has been unsubscribed from the next topics: ${topicUnsubscriptionList}`
                );
            })
            .catch((error) =>
                respondWithFailure(clientId, error.message, message)
        ...
        respondWithFailure(
            clientId,
            `Payload should consist property "t" with list of topics to unsubscribe`,
            message
        );
    }
}
...
function processPluginAuthenticateAction(clientId, message) {
    if (message.payload && message.payload.token) {
        pluginManager
            .authenticate(clientId, message.payload.token)
            .then((tokenPayload) => {
                webSocketServer.send(
                    clientId,
                    new Message({
        ...
        respondWithFailure(
            clientId,
            `Payload should consist property "token" to authenticate plugin`,
            message
        );
    }
}
