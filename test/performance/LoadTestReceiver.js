...
const { Message, MessageUtils, MessageBuilder } = require(`devicehive-proxy-message`);
...
process.on(`message`, (message) => {
    switch (message.action) {
        case "start":
            startReceiverRoutine(message.uuid);
            break;
    }
});

function startReceiverRoutine(uuid) {
    const proxyClient = new ProxyClient(Config.PROXY_SERVER_URL);

    proxyClient.on(`open`, async () => {
        await proxyClient.sendMessage(new Message({
            type: MessageUtils.TOPIC_TYPE,
            action: MessageUtils.CREATE_ACTION,
            payload: {topicList: [Config.TEST_TOPIC + uuid]}
        }));
    ...
    proxyClient.on(`message`, (message) => {
        if (message.type === MessageUtils.NOTIFICATION_TYPE && message.action !== MessageUtils.CREATE_ACTION) {
            ...
            if (rCount === TOTAL_MESSAGES) {
                ...
                process.send({
                    action: `finished`,
                    averageThroughput: throughputList.reduce((p, t) => p + (t / throughputList.length), 0)
                });
            }
        }
    });
