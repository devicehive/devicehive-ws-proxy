...
class ExtConsumerGroup extends kafka.ConsumerGroup {
  ...
  constructor(config, topics) {
    ...
    me._mq = new Mq();
    //me.offsetsMap = new Map();
  }

  // isLatestOffset ({ topic, partition, offset}) {
  //     const me = this;
  //
  //     return !me.offsetsMap.has(topic) || me.offsetsMap.get(topic)[partition].offset < offset;
  // }
  //
  // setLatestOffset ({ topic, partition, offset}) {
  //     const me = this;
  //     const offsetsObj = me.offsetsMap.has(topic) ? me.offsetsMap.get(topic) : {};
  //
  //     offsetsObj[partition] =  { offset: offset };
  //     me.offsetsMap.set(topic, offsetsObj)
  // }
...
class ExtProducer extends kafka.Producer {
  ...
  _sendingCallback(error, data){
    debug(`CALLBACK ON PRODUCER SEND`);
