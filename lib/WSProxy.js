...
class WSProxy extends EventEmitter {
  ...
  start() {
    ...
    this.webSocketServer
      .on(`connection`, (ws, req) => {
        ...
        ws.clientType = req.headers[`client-type`];
        try {
          ws
      ...
      try {
        messageData = Msg.fromJSON(message);
          ...
          if (ws.producer.isFull){
              ...
              if (ws.clientType === `dh-service` || ws.authenticated === true) {
                  const replyMessage = Msg.createReplyMessage({
	                  id: messageData.id,
	                  t: Msg.TYPE_ACK
                  });
                  ...
                  if (this.isPluginAuthenticationRequest(messageData)) {
	                  this._handleSingleMsg(messageData, ws);
                  } else {
	                  const replyMessage = Msg.createReplyMessage({
		                  t: Msg.TYPE_ACK
	                  ...
	                  replyMessage.status = Msg.STATUS_FAIL;
	                  this._setPayloadAndSend(`Not authorized`, replyMessage, ws);
                  }
              }
          }
      } catch (error) {
  ...
  static _errorOrDataCallback(error, data) {
	...
	_handlePluginMessage(ws, message) {
		...
		if (message.action === Msg.ACTION_AUTHENTICATE) {
			request({
				method: `GET`,
				uri: `http://localhost:8090/dh/rest/token/plugin/authenticate?token=${JSON.parse(message.p).token}`,
				...
				replyMessage.status = ws.authenticated = !err && response.statusCode === 200 ? Msg.STATUS_SUCCESS : Msg.STATUS_FAIL;
