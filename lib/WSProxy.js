...
class WSProxy extends EventEmitter {
  ...
  _initProducer(ws) {
    ...
    producer
      ...
      .on(`send-batch`, (data) => {
        this._sendMessagesToBrocker(producer, data);
      });
  ...
  /**
   * Internal! Send message queue to Brocker
   * 
   * @param {Object} producer 
   * @param {Mq} mq 
   * @memberof WSProxy
   */
  _sendMessagesToBrocker(producer, mq) {
    const map = {};
    mq.forEach(message => {
    ...
    debug(payload);
    producer.send(payload, WSProxy._errorOrDataCallback.bind(producer));
  }
