...
const APP_CONFIG = require(`../application/config.json`);
...
class WSProxy extends EventEmitter {
  ...
  get isAvailable(){
	...
	start() {
		this.webSocketServer = new WebSocket.Server({
			verifyClient: (clientInfo) => {
				clientInfo.req.authenticated = [`dh-service`].includes(clientInfo.req.headers[`client-type`]);
				return true;
			},
		...
		this.webSocketServer
			.on(`connection`, (ws, req) => {
				ws.authenticated = req.authenticated;
	...
	*/
	_handleWebSocketMessage(message, ws) {
	    ...
	    try {
		    messageObject = Msg.fromJSON(message);

	    	if (!me.isAvailable) { throw new Error(`Proxy not available`) }
	    	if (ws.producer.isFull) { throw new Error(`Message Queue is full`) }

		    me.emit(`ws-message`, message);

		    if (ws.authenticated === true) {
			    const replyMessage = Msg.createReplyMessage(
			    	{ id: messageObject.id, t: Msg.TYPE_ACK },
				    { status: Msg.STATUS_SUCCESS });

			    me._setPayloadAndSend({}, replyMessage, ws);

			    Array.isArray(messageObject) ?
				    messageObject.forEach((messageItem) => me._handleSingleMsg(messageItem, ws)) :
				    me._handleSingleMsg(messageObject, ws);
		    } else {
			    if (!me.isPluginAuthenticationRequest(messageObject)) { throw new Error(`Not authorized`) }

			    me._handleSingleMsg(messageObject, ws);
		    }

	    } catch (error) {
		    const replyMessage = Msg.createReplyMessage({ t : Msg.TYPE_ACK }, { status: Msg.STATUS_FAIL });

		    me._raiseError(error);
		    ...
		    me._setPayloadAndSend(error.message, replyMessage, ws);
	    }
	}
	 ...
	 * @returns {boolean}
	 */
	isPluginAuthenticationRequest(messageData) {
        return messageData.t === Msg.TYPE_PLUGIN && messageData.a === Msg.ACTION_AUTHENTICATE;
    }
   ...
   */
	_handleSingleMsg(message, ws) {
		if (message.isPluginTopic) {
			this._handlePluginMessage(ws, message);
		} else if (message.isTypeTopic) {
  ...
  static _errorOrDataCallback(error, data) {
	...
	/**
	 * Plugin messages handler
	 * @param ws
	 * @param message
	 * @private
	 */
	_handlePluginMessage(ws, message) {
		const me = this;
		const replyMessage = Msg.createReplyMessage({ id: message.id, t: Msg.TYPE_ACK, a: message.a });

		try {
			switch (message.action) {
				case Msg.ACTION_AUTHENTICATE:
					const token = JSON.parse(message.p).token;

					request({
						method: `GET`,
						uri: `${APP_CONFIG.AUTH_SERVICE_ENDPOINT}/token/plugin/authenticate?token=${token}`
					}, (err, response, body) => {
						ws.authenticated = !err && response.statusCode === 200;
						replyMessage.status =  ws.authenticated  ?
							Msg.STATUS_SUCCESS : Msg.STATUS_FAIL;
						me._setPayloadAndSend(body, replyMessage, ws);
					});
					break;
				default:
                    throw new Error(`Plugin Action: ${message.a} not supported`);
					break;
			}
		} catch (error) {
