...
class WSProxy extends EventEmitter {
  ...
  _sendMessagesToBroker(producer, mq) {
      ...
      Promise.all(mq.map((message) => new Promise((resolve) => {
          producer.client.loadMetadataForTopics([message.payload.t], function (error, results) {
              const partitionsAmount = Object.values(results[1].metadata[message.payload.t]).length;
