...
const kafkaext = require(`./kafka-extension`);
const EventEmitter = require(`events`);
const Msg = require(`./msg`);
const debug = require(`debug`)(`ws-kafka:proxy`);

class WSKafkaProxy extends EventEmitter {
      ...
      .on('connection', (ws, req) => {
        ...
        try {
          ws.on('close', (code, reason) => {
            ...
            ws.shutDown(WSKafkaProxy._errorOrDataCallback);
          }).on('pong', () => {
            try {
              ws.isAlive = true;
  ...
  _initProducer(ws) {
    ...
    producer
      .on('ready', () => {
        WSKafkaProxy._resumeWsWhenKafkaReady(ws);
  ...
  _sendMessagesToKafka(producer, mq) {
    ...
    debug(payload);
    producer.send(payload, WSKafkaProxy._errorOrDataCallback);
  }
  ...
  _handleUnsubscribeTopics(ws, msg, rm) {
    ws.shutDownConsumer(WSKafkaProxy._errorOrDataCallback);
  ...
  _initConsumerAndSubscribeTopics(ws, msg) {
    ...
    ws.shutDownConsumer();
    this._createConsumerGroup(ws, payload.t, msg);
  }
...
module.exports.WSKafkaProxy = WSKafkaProxy;